...
class TestPredictionRequest:
    ...
    def test_integer_user_id_converted_to_string(self):
        ...
        Pydantic v2 strict str fields do NOT auto-coerce int to string.
        """
        with pytest.raises(ValidationError):
...
class TestHealthResponse:
    ...
    def test_health_response_status_types(self):
        ...
        """
        # Test healthy status
        healthy = HealthResponse(status="healthy", model_loaded=True)
        assert healthy.status == "healthy"

        # Test unhealthy status
        unhealthy = HealthResponse(status="unhealthy", model_loaded=False)
        assert unhealthy.status == "unhealthy"

        # Test custom status
        custom = HealthResponse(status="degraded", model_loaded=True)
...
class TestBatchPredictionRequest:
    ...
    def test_valid_batch_request(self):
        ...
        assert request.predictions[1].movie_id == "302"
    ...
    def test_too_many_predictions_raises_error(self):
        ...
        """
        predictions = [PredictionItem(user_id=str(i), movie_id=str(i + 1)) for i in range(101)]
        with pytest.raises(ValidationError):
